...
class Master(Canvas):
    ...
    def __init__(self, boss=None):
        super().__init__(boss)
        self.configure(width=800, height=600, bg=BG_COLOR)
        self.running = 0
        self.snake = None
        self.obstacle = None
        self.direction = None
    ...
    def clean(self):
        ...
        if self.running == 1:
            global final_score
            final_score = self.score.counter.get()
            # print(final_score.get())
            self.score.reset()
            self.current.stop()
            self.running = 0
...
class Snake:
    ...
    def move(self, path):
        ...
        a = (self.blocks[-1].x + STEP * path[0]) % 800
        b = (self.blocks[-1].y + STEP * path[1]) % 600
...
if __name__ == "__main__":
    ...
    def start():
        ...
        game_sn = Master(root)
        game_sn.pack(side=BOTTOM)
        root.bind("<Key>", game_sn.redirect)
        # while game_sn.running == 1:
        game_sn.start()
        # root.lift()
        game.withdraw()
        root.focus_force()
        # global scorecard
        scoreboard = Frame(root, width=35, height=40)
        Label(scoreboard, text='Game Score', font=("Arial", 13)).grid(column=0, row=0)
        Label(scoreboard, textvariable=game_sn.score.counter, font=("Arial", 13)).grid(column=2, row=0)
        scoreboard.pack(pady=10, padx=0, side=TOP)


        # print(game_sn.running)

    def final_screen():
        root.destroy()
        final_game = Tk()
        final_game.title("Snake Game")
        final_game.geometry("800x600")
        final_game.config(bg='black')

        Label(final_game, text='Game Over', bg='black', fg='red', font=("Arial", 45, 'bold')).pack(pady=(60,40))
        Label(final_game, text=f'Final Score: {final_score}', bg='black', fg='red', font=("Arial", 45, 'bold')).pack(pady=20)

        def restart():
            final_game.destroy()
            start()
        Button(final_game, text='Restart', bg='red', fg='black', width=10, font=("Arial", 20, 'bold'),
        ...
        Button(final_game, text='Home', bg='red', fg='black', width=10, font=("Arial", 20, 'bold'), command=go_home).place(x=480, y=320)

        final_game.mainloop()
    def close():
        game.destroy()

    Label(game, text='Home Page', bg='black', fg='red', font=("Arial", 40, 'bold')).pack(pady=30)

    Button(game, text='Start', bg='red', fg='black', width=10, font=("Arial", 20, 'bold'), command=start).place(x=150, y=200)

    Button(game, text='Close', bg='red', fg='black', width=10, font=("Arial", 20, 'bold'), command=close).place(x=480, y=200)
